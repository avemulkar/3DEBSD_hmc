function [filtered,original] = quaternion_kuwahara(path, passes,headerlines)    
...
[M] = [phi1 PHI phi2 xpos ypos];
[Cx,I] = max(xpos);
[Cy] = max(ypos);
stepsize = xpos(I)-xpos(I-1);

%% Prepare a Matrix for kuwahara filter
X = zeros(round(Cy/stepsize)+1,round(Cx/stepsize)+1,3);
for i = 1:(length(M(:,1)))
         y = 1+M(i,5)/stepsize;%this is the y index in the row
         x = 1+M(i,4)/stepsize;%this is the x index in the row
         X(round(y),round(x),:) = [M(i,1),M(i,2),M(i,3)];
        
end%for

%% Convert to Quaternion
Q = zeros(round(Cy/stepsize)+1,round(Cx/stepsize+1),4);
for j = 1:(length(X(:,1,1))) %for y position
    for i = 1:(length(X(1,:,1))) %for x position       
        alpha = X(j,i,1);
        beta = X(j,i,2);
        gamma = X(j,i,3);
...
%         else
            ...
            b = -sin((alpha-gamma)./2) .* sin(beta./2);
            c = cos((alpha-gamma)./2) .* sin(beta./2);
